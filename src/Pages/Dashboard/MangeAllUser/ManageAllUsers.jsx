...
const ManageAllUsers = () => {
    const { data: users = [], refetch } = useQuery(['users'], async () => {
        const res = await fetch('https://musician-instrument-school.vercel.app/users')
        return res.json()
    })
    const handleMakeAdmin = user => {
        fetch(`https://musician-instrument-school.vercel.app/users/admin/${user._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
    ...
    const handleInstructor = user => {
        fetch(`https://musician-instrument-school.vercel.app/users/instructor/${user._id}`, {
            method: 'PATCH'
        })
            .then(res => res.json())
            .then(data => {
